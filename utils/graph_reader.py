...
#         self.max_iter = (len(self.filedata) // batch_size)
    ...
    def __init__(self, graph_path):
        ...
        self.gFilenames = []
        for entry in os.listdir(graph_path):
            self.gFilenames.append(entry)
...
#             yield self.process_batch(self.filedata[start_idx:end_idx])
    ...
    def __iter__(self):
        for entry in self.gFilenames:
            gFilename = os.path.join(self.graph_path, entry)
            graph = np.loadtxt(gFilename)
            yield self.process_batch(entry, graph)
...
#         return batch
    ...
    def process_batch(self, gFilename, graph):
        """Helper function to convert raw lines into a mini-batch as a DotDict.
        """
        batch_edges = []
        batch_edges_values = []
        ...
        nodes = np.ones(graph.shape[0])
        
        # Compute distance matrix
        W_val = squareform(pdist(graph, metric='euclidean'))

        # Compute adjacency matrix
#         if self.num_neighbors == -1:
        W = np.ones((graph.shape[0], graph.shape[0]))  # Graph is fully connected
#         else:
#             W = np.zeros((graph.shape[0], graph.shape[0]))
#             # Determine k-nearest neighbors for each node
#             knns = np.argpartition(W_val, kth=self.num_neighbors, axis=-1)[:, self.num_neighbors::-1]
#             # Make connections 
...
#             batch_edges_target.append(edges_target)
        batch_nodes.append(nodes)
#             batch_nodes_target.append(nodes_target)
        batch_nodes_coord.append(graph)
#             batch_tour_nodes.append(tour_nodes)
#             batch_tour_len.append(tour_len)
